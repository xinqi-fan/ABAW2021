...
from data import AffWild2AUDataset
from models.resnet50_ft_dag import Resnet50_ft_dag
from utils import AverageMeter, accuracy, AU_metric
...
def parse_arguments():
    ...
    parser.add_argument('--task', type=str, default='AU')
...
def set_loader(args):
    ...
    if args.dataset == 'Aff-Wild2':
        train_dataset = AffWild2AUDataset(args.data_folder, phase='train', transform=train_transform)
        val_dataset = AffWild2AUDataset(args.data_folder, phase='validation', transform=val_transform)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for idx, (images, targets) in enumerate(train_loader):
        ...
        correct_sum = sum(predict == targets).sum()
        acc_batch = correct_sum.float()/(bsz*args.n_cls)
        acc.update(acc_batch, bsz)
        label['gt'].append(targets.cpu().detach().numpy())
        label['pred'].append(predict.cpu().detach().numpy())
